...
32. Use "needs_human" ONLY for tasks that are truly IMPOSSIBLE without credentials: entering passwords, payment details, private keys, or 2FA codes the user must provide. Do NOT give up on visual challenges (CAPTCHAs, image selection, puzzles) — say "cannot_read" instead so the vision fallback can try. ALWAYS attempt the task first. Only escalate to needs_human after the ENTIRE pipeline has failed.
...
36. When you see visual content you cannot read from OCR text alone (CAPTCHA images, image-based puzzles, graphs, charts, screenshots-within-screenshots), return "cannot_read" so the vision model can see and interpret the visual content. Do NOT guess or hallucinate what visual content shows — if OCR can't read it, say so.`;
